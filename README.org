...
Imagine a function as a box. You give the box your input(s), and it spits out your output. Each set of inputs corresponds with only one output. This means that every time you throw identical inputs into the box, you can expect it to spit out the same output.
